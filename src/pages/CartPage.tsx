...
import { IconUAH } from '../components/IconUAH'
import { useAppDispatch } from '../store/hooks';
import { clearOrder } from '../store/slices/cartSlice';

const { CartEmpty } = lazily(() => import('../components/Cart/CartEmpty'));
...
export const CartPage: React.FC<CartPageProps> = ({ totalPrice, totalCount, cart }) => {
   const dispatch = useAppDispatch()
   const navigate = useNavigate()
   const cart_items: CartItemType[] = Object.values(cart).reduce(
      (acc: any, val: any) => acc.concat(val.items[0]), []
   )

   const pizzaNames: string[] = []
   cart_items.forEach((item: CartItemType) => pizzaNames.push(item.name))
   const order = pizzaNames.join(', ')

   const interactionWithUser = (msg: string, variant: string = '') => {
      if(window.confirm(msg)){
         if (variant === 'pay') alert(`Ваш заказ: ${order}`)
         dispatch(clearOrder())
         navigate('/')
      }
   }
   ...
   const handleOrderPay = () => {
      interactionWithUser('Вы действительно хотите перейти к оплате заказа?', 'pay')
   }
   const handleClearCart = () => {
      interactionWithUser('Вы действительно хотите очистить корзину?')
   }
   return (
      <section className='cart'>
         {Object.keys(cart).length
            ? <div className="cart__body">
               <div className="cart__top top-cart">
                  <div className="top-cart__inner">
                     <div className="top-cart__title">
                        ...
                        <h2 className="top-cart__title-text">Корзина</h2>
                     </div>
                     <button onClick={handleClearCart} className="top-cart__clear">
               ...
               {cart_items.length && cart_items.map((item: CartItemType, idx: number) => (
                  <CartItem
                     key={idx} 
                     {...item}
                     // @ts-ignore
                     count={cart[item.id].totalCount}
                     // @ts-ignore
                     price={cart[item.id].totalPrice}
                     // @ts-ignore
                     allSpecies={cart[item.id].allSpecies}
                  />
               ))}
               <div className="cart__total total-cart">
                  <div className="total-cart__inner">
                     <div className="total-cart__total-count">Всего пицц: <span>{totalCount} шт.</span></div>
                     <div className="total-cart__total-sum">Сумма заказа:
                        <span>{totalPrice}</span>
                        <span><IconUAH color={'#FE5F1E'} className={'total-cart__sum-icon'} /></span>
                     </div>
                  </div>
               </div>
               <div className="cart__actions actions-cart">
                  <div className="actions-cart__inner">
                     <BackButton />
                     <Link to={'/'}>
                        <Button 
                           onClickFn={handleOrderPay} 
                           className="actions-cart__pay-btn pay-btn"
                        >
            ...
            : <React.Suspense fallback={<h1>Идёт загрузка...</h1>}><CartEmpty /></React.Suspense>
         }
      </section>
   )
}